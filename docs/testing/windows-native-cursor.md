...
This script does not launch Screenly. It:
...
```text
C:\Users\<user>\AppData\Local\Temp\screenly-cursor-native-...
```
...
$env:CURSOR_TEST_OUTPUT_DIR = "C:\temp\screenly-cursor-test"
npm run test:cursor-native:win
```

## Screenly preview capture

```powershell
npm run capture:screenly-preview
```

This script launches the real Electron app, injects a fixture video plus cursor sidecar data, opens the editor, captures frames from the actual Screenly preview UI, and encodes them into a WebM.
...
```powershell
$env:SCREENLY_PREVIEW_SKIP_BUILD = "true"
$env:SCREENLY_PREVIEW_FRAME_COUNT = "120"
$env:SCREENLY_PREVIEW_FPS = "30"
$env:SCREENLY_PREVIEW_OUTPUT_DIR = "C:\temp\screenly-preview"
npm run capture:screenly-preview
```

Useful files:

- `screenly-preview.webm`: video of the real Screenly editor preview
...
- whether the real Screenly preview renders the same cursor behavior as the diagnostic pipeline
...
Build Screenly's helper locally:
...
For local diagnostics with another compatible helper, point Screenly at that executable:

```powershell
$env:SCREENLY_WGC_CAPTURE_EXE = "C:\path\to\wgc-capture.exe"
npm run build-vite
npm run dev
```
