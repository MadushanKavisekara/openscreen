...
AudioInputFormat makeAacCompatibleAudioFormat(const AudioInputFormat& sourceFormat) {
    AudioInputFormat normalized{};
    if (sourceFormat.sampleRate == 0 || sourceFormat.channels == 0) {
        return normalized;
    }

    normalized.subtype = MFAudioFormat_PCM;
    normalized.sampleRate = sourceFormat.sampleRate;
    normalized.channels = sourceFormat.channels > 1 ? 2 : 1;
    normalized.bitsPerSample = 16;
    normalized.blockAlign = normalized.channels * static_cast<UINT32>(sizeof(int16_t));
    normalized.avgBytesPerSec = normalized.sampleRate * normalized.blockAlign;
    return normalized;
}
...
bool MFEncoder::configureAudioStream(const AudioInputFormat& audioFormat) {
    ...
    if (encoderFormat.sampleRate == 0 || encoderFormat.channels == 0 || encoderFormat.blockAlign == 0) {
    ...
    setAudioFormat(
        inputType.Get(),
        encoderFormat.channels,
        encoderFormat.sampleRate,
        encoderFormat.bitsPerSample);
