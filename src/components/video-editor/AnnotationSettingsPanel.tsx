			...
			<div className="mb-3">
				...
				>
					<TabsList className="mb-4 bg-white/[0.035] border border-white/[0.06] p-0.5 w-full grid grid-cols-3 h-9 rounded-xl">
						<TabsTrigger
							value="text"
							className="data-[state=active]:bg-brand data-[state=active]:text-white text-slate-400 rounded-lg transition-all gap-1.5 text-[11px]"
						>
						...
						<div>
							...
							<Popover>
								<PopoverTrigger asChild>
									...
									>
										<div
											...
											style={{ backgroundColor: annotation.figureData?.color || "#3A7DFF" }}
										/>
										<span className="text-xs text-slate-300 truncate flex-1 text-left">
											{annotation.figureData?.color || "#3A7DFF"}
										</span>
										<ChevronDown className="h-3 w-3 opacity-50" />
									</Button>
								</PopoverTrigger>
								<PopoverContent className="w-[260px] p-3 bg-[#1a1a1c] border border-white/10 rounded-xl shadow-xl">
									<Block
										color={annotation.figureData?.color || "#3A7DFF"}
										colors={colorPalette}
										onChange={(color) => {
											const newFigureData: FigureData = {
												...annotation.figureData!,
